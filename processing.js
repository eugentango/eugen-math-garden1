...
async function loadModel() {
  //as our model expects a 2d array: 1 by 784 - [[]]
  ...
  model = await tf.loadGraphModel('TFJS1/model.json'); // await - wait the model to load
  // console.log(model);
  // const result = model.predict(myTensor);
  // result.print();
}
//my solution, works
// async function predictModel(myData) {

//   const model2 = await tf.loadGraphModel('TFJS/model.json'); // await - wait the model to load
//   const result2 = model2.predict(myData);
//   alert(result2);
// }
...
function predictImage() {
  ...
  //let - defining a variable locally within function
  ...
  //Center of mass - similar to the previous steps but the image is now different
  cv.findContours(image, contours, hierarchy, cv.RETR_CCOMP, cv.CHAIN_APPROX_SIMPLE);
  cnt = contours.get(0);
  ...
  //shifting our image according to the center of mass 
  //as the image is 28 px - mean = 14
  const X_SHIFT = Math.round(image.cols / 2.0 - cx);
  const Y_SHIFT = Math.round(image.rows / 2.0 - cy);

  const M = cv.matFromArray(2, 3, cv.CV_64FC1, [1, 0, X_SHIFT, 0, 1, Y_SHIFT]);
  newSize = new cv.Size(image.rows, image.cols);
  cv.warpAffine(image, image, M, newSize, cv.INTER_LINEAR, cv.BORDER_CONSTANT, BLACK);//new cv.Scalar()); - reusing old scalar

  //Normalizing values using map-method
  let pixelValues = image.data;//gives an array of integers (we need floats)
  ...
  //Creating a tensor
  //for a tensot we need [[]], one is already in pixelValues
 ...
 // predictModel(X);
  ...
  //getting result of a tensor
  //sync method returns an array. here, we need only the first element
  ...
  hierarchy.delete();
  M.delete();
  //cleaning our tensors
  X.dispose();
  result.dispose();

  //function is executed till the return-statement 
  return output;
}
